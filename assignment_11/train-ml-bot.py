...
def create_dataset(path, player=alphabeta.Bot(), games=2000, phase=2):
...
if options.overwrite or not os.path.isfile(options.dset_path):
    create_dataset(options.dset_path, player=alphabeta.Bot(), games=10000)
...
if options.train:
    ...
    joblib.dump(model, "./bots/ml/alphabeta_" + options.model_path)
